...
cutoff = 2  # Define the cutoff for outliers

for data_name, data_frame in UOP_data.items():
    numeric_columns = data_frame.select_dtypes(include=['float64', 'int64']).columns
    zscored_data = data_frame[numeric_columns].apply(zscore)

    # Cap outliers
    capped_data = np.where(np.abs(zscored_data) > cutoff, np.sign(zscored_data) * cutoff, zscored_data)

    # Assign capped data back to the original data frame
    UOP_data[data_name][numeric_columns] = capped_data
