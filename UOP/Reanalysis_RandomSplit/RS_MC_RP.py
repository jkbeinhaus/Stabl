...
for data_name, data_frame in STA_data.items():
    ...
    STA_data[data_name][numeric_columns] = STA_data[data_name][numeric_columns].apply(zscore)
...
UOP_y['site'] = 'UOP'
X_Celldensities = pd.concat([STA_Celldensities, UOP_Celldensities])
X_Function = pd.concat([STA_Function, UOP_Function])
X_Metavariables = pd.concat([STA_Metavariables, UOP_Metavariables])
X_Neighborhood = pd.concat([STA_Neighborhood, UOP_Neighborhood])
